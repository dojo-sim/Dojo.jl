function getant(; Δt::T=0.05, g::T=-9.81, cf::T=0.5,
    ...
    if contact
        ...
        contineqcs = [contact_constraint(foot[i], normal, cf=cf, p=p[i], offset=o[i]) for i = 1:length(foot_names)]
        ...
        if contact_body
            ...
            torso_ineqcs = contact_constraint(torso, normal, cf=cf, p=p, offset=o)
            ...
            elbow_ineqcs = [contact_constraint(elbow[i], normal, cf=cf, p=p[i], offset=o[i]) for i = 1:length(elbow_names)]
