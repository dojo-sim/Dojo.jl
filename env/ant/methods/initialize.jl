...
function initializeant!(mechanism::Mechanism; ankle = 0.25, alt=0.15, pos=[0.0; 0.0; 0.48 + alt], rot=[0.0; 0.0; 0.00 * π]) where {T}
    ...
    for i in [1,4]
        setPosition!(mechanism, geteqconstraint(mechanism, Symbol("hip_$i")), 0.0*[0.0 * π])
        setPosition!(mechanism, geteqconstraint(mechanism, Symbol("ankle_$i")), [ankle * π])
    end

    for i in [2,3]
        setPosition!(mechanism, geteqconstraint(mechanism, Symbol("hip_$i")), [0.0 * π])
        setPosition!(mechanism, geteqconstraint(mechanism, Symbol("ankle_$i")), [-ankle * π])
    end
