...
function AtlasIKerror(mechanism::Mechanism, p_base, p_foot, θ; leg::Symbol = :r)
	set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :auto_generated_floating_joint), [p_base; zeros(3)])
	set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, Symbol(leg, :_leg_hpxyz)), [0.0, -θ[1], 0.0])
	set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, Symbol(leg, :_leg_kny)), [θ[2]])
	set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, Symbol(leg, :_leg_akxy)), [θ[1]-θ[2], 0.0])
...
function atlas_trajectory(mechanism::Mechanism{T}; timestep = 0.05, β=0.5,
	...
	for i = 1:2N
		...
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :auto_generated_floating_joint), [p_base; 0.0; αtorso; 0.0])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :back_bkxyz), [0.0, -αtorso, 0.0])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :l_leg_hpxyz), [0.0, -θL[i][1], 0.0])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :l_leg_kny), [θL[i][2]])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :l_leg_akxy), [θL[i][1]-θL[i][2], 0.0])

		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :r_leg_hpxyz), [0.0, -θR[i][1], 0.0])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :r_leg_kny), [θR[i][2]])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :r_leg_akxy), [θR[i][1]-θR[i][2], 0.0])

		set_minimal_velocities!(mechanism, get_joint_constraint(mechanism, :auto_generated_floating_joint), [zeros(3); zeros(3)])
		set_minimal_velocities!(mechanism, get_joint_constraint(mechanism, :l_leg_hpxyz), [0.0, -ωL[i][1], 0.0])
		set_minimal_velocities!(mechanism, get_joint_constraint(mechanism, :l_leg_kny), [ωL[i][2]])
		set_minimal_velocities!(mechanism, get_joint_constraint(mechanism, :l_leg_akxy), [ωL[i][1]-ωL[i][2], 0.0])

		set_minimal_velocities!(mechanism, get_joint_constraint(mechanism, :r_leg_hpxyz), [0.0, -ωR[i][1], 0.0])
		set_minimal_velocities!(mechanism, get_joint_constraint(mechanism, :r_leg_kny), [ωR[i][2]])
		set_minimal_velocities!(mechanism, get_joint_constraint(mechanism, :r_leg_akxy), [ωR[i][1]-ωR[i][2], 0.0])
...
function atlas_jump(mechanism::Mechanism{T}; timestep = 0.05, αtorso=0.0, z=0.95, Δx=0.0, x = 0.0,
	...
	for i = 1:N
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :auto_generated_floating_joint), [pbase[i]; 0.0; αtorso; 0.0])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :back_bkxyz), [0.0, -αtorso, 0.0])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :l_leg_hpxyz), [0.0, -θL[i][1], 0.0])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :l_leg_kny), [θL[i][2]])
		set_minimal_coordinates!(mechanism, get_joint_constraint(mechanism, :l_leg_akxy), [θL[i][1]-θL[i][2], 0.0])
