...
## setup 
xa = [-1.0; 0.0; 0.0]
qa = UnitQuaternion(RotY(0.0 * π) * RotX(0.0 * π))
...
# set configuration
settransform!(vis[:cap1], compose(Translation(xa), LinearMap(UnitQuaternion(qa..., false))))
settransform!(vis[:cap2], compose(Translation(xb), LinearMap(UnitQuaternion(qb..., false))))
