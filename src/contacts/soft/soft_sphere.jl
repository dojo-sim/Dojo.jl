...
function inside(collision::SoftSphereCollision{T}, p) where T
    origin = collision.contact_origin
    radius = collision.contact_radius
    c = norm(p - origin)
    # return c <= radius
    # @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    return false
...
function contact_normal(collision::SoftSphereCollision, p, xc, qc)
    # center of the sphere expressed in the world frame
    pc = xc + Dojo.vector_rotate(collision.contact_origin, qc)
    normal = p - pc
