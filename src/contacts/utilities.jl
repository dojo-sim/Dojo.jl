...
function get_sdf(contact::ContactConstraint{T,N,Nc,Cs}, x::AbstractVector{T},
    ...
    model = contact.model
    return model.collision.surface_normal_projector * (x + vector_rotate(model.collision.contact_point, q)) - model.collision.contact_radius
end
...
function contact_location(contact::ContactConstraint{T,N,Nc,Cs}, x::AbstractVector{T},
    q::UnitQuaternion{T}) where {T,N,Nc,Cs<:Contact{T,N}}
    model = contact.model
    return x + vector_rotate(model.collision.contact_point, q) - model.collision.surface_normal_projector' * model.collision.contact_radius 
end
