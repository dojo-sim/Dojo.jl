...
function contact_constraint(bodies::Vector{Body{T}},
    ...
    n = length(normal)
    @assert n == length(bodies) == length(normal) == length(friction_coefficient) == length(contact_points) == length(contact_radius)
...
function contact_constraint(body::Body{T},
        ...
        contact_type::Symbol=:nonlinear) where T
    n = length(normal)
    @assert n == length(normal) == length(friction_coefficient) == length(contact_points) == length(contact_radius)
