mutable struct JointBound{T,N} <: Bound{T,N}
    ainv3::Adjoint{T,SVector{3,T}} # inverse matrix
    offset::SVector{3,T}

    function JointBound(body::Body{T}, offset::AbstractVector = szeros(T, 3)) where T
...
function impactconstraint(body::Body{T}, normal::AbstractVector{T};
    ...
    impbound = JointBound(body, normal, p = p, offset = offset)
    impineqcs = InequalityConstraint((impbound, body.id, nothing))
    return impineqcs
end

function g(mechanism, ineqc::InequalityConstraint{T,N,Nc,Cs}) where {T,N,Nc,Cs<:Tuple{JointBound{T,N}}}
...
@inline function ∂g∂pos(bound::JointBound, x::AbstractVector, q::UnitQuaternion)
    X = bound.ainv3;
...
## Complementarity
function complementarity(mechanism, ineqc::InequalityConstraint{T,N,Nc,Cs,N½}; scaling = false) where {T,N,Nc,Cs<:Tuple{JointBound{T,N}},N½}
    γ = ineqc.γsol[2]
    s = ineqc.ssol[2]
    if scaling
...
function complementarityμ(mechanism, ineqc::InequalityConstraint{T,N,Nc,Cs,N½}; scaling = false) where {T,N,Nc,Cs<:Tuple{JointBound{T,N}},N½}
    γ = ineqc.γsol[2]
    s = ineqc.ssol[2]
    if scaling
        W, Wi, λ = nt_scaling(ineqc)
...
function neutral_vector(bound::JointBound{T,N}) where {T,N}
    N½ = Int(N/2)
    return sones(T, N½)
end

@inline function ∂g∂ʳpos(bound::JointBound, x::AbstractVector, q::UnitQuaternion)
    X, Q = ∂g∂pos(bound, x, q)
    return [X Q]
end

@inline function ∂g∂ʳvel(bound::JointBound, x3::AbstractVector, q3::UnitQuaternion,
...
@inline function setDandΔs!(mechanism::Mechanism, matrix_entry::Entry, vector_entry::Entry,
    ineqc::InequalityConstraint{T,N,Nc,Cs,N½}) where {T,N,Nc,Cs<:Tuple{JointBound{T,N}},N½}
...
function nt_scaling(ineqc::InequalityConstraint{T,N,Nc,Cs,N½}) where {T,N,Nc,Cs<:Tuple{JointBound{T,N}},N½}
    γ = ineqc.γsol[2]
    s = ineqc.ssol[2]
...
# cone degree
function cone_degree(bound::JointBound{T,N}) where {T,N}
