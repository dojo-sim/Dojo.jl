...
abstract type Space28{T,N} end

mutable struct BoxSpace28{T,N} <: Space28{T,N}
    n::Int # box dimension
    low::AbstractVector{T} # minimum value
    high::AbstractVector{T} # maximum value
end

function BoxSpace28(n::Int; low::AbstractVector{T} = -ones(n), high::AbstractVector{T} = ones(n)) where {T}
    return BoxSpace28{T,n}(n, low, high)
end

function sample(s::BoxSpace28{T,N}) where {T,N}
    return rand(T,N) .* (s.high .- s.low) .+ s.low
end

function contains(s::BoxSpace28{T,N}, v::AbstractVector{T}) where {T,N}
...
mutable struct Environment28{M,A,O}
    mechanism::M
    model::Symbol
...
function Environment28(mechanism::Mechanism, model::Symbol, reward_fct::Any; max_iter::Int = 100)
    iter = 0
    aspace = BoxSpace28(controldim(mechanism))
    ospace = BoxSpace28(maxCoordDim(mechanism))
    TYPE = [typeof(mechanism), typeof(aspace), typeof(ospace)]
    env = Environment28{TYPE...}(mechanism, model, reward_fct, iter, max_iter, aspace, ospace)
    return env
end
...
function make(model::String; kwargs...)
    ...
    env = Environment28(mechanism, model, reward_fct)
    reset(env)
    return env
end

function reset(env::Environment28{M}; kwargs...) where {M}
    env.iter = 0
...
function render(env::Environment28{M}; kwargs...) where {M}

    return nothing
end

function close(env::Environment28{M}; kwargs...) where {M}
    # visualizer stuff
    return nothing
end

function need_to_reset(env::Environment28)
    done = env.iter >= env.max_iter - 1
    return done
end

function step(env::Environment28{M}, action) where {M}
    env.iter += 1
