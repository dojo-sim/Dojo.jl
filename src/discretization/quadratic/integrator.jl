## Uses quadratic interpolation between the knot points and a point in between the knot points. The integration follows Simpson's rule.
## 
# u(t) = a t^2 + b t + c
# L(xck,vck) -> Δt/6 (Ld(xdk,(xdk+1-xdk)/Δt) + 4 Ld(xdk+1,(xdk+1-xdk)/Δt)), where a + b = 1
# L(qck,ωck) -> Δt (a Ld(qdk,2 V qdk† (qdk+1-qdk)/Δt) + b Ld(qdk+1,2 V qdk† (qdk+1-qdk)/Δt), where a + b = 1
