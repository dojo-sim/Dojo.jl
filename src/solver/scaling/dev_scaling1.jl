...
function nd_scaling(s, z)
    ...
    W̄ = 2 * v * v' - J
    W̄i = 2 * J * v * v' * J - J
    # @assert norm(W̄ * (2J*ωb*ωb'*J - J) * W̄ - I(n), Inf) < 1e-8
...
function nd_λ(s, z)
    ...
    r = sqrt((s'*J*s)/(z'*J*z))
    W = sqrt(r) * W̄
    # @assert norm(λb - W̄i * s̄, Inf) < 1e-8
    # @assert norm(λb - J*W̄*J*s̄, Inf) < 1e-8
    # @assert norm(λb'*J*λb - 1, Inf) < 1e-8
    # @assert norm(W̄i - inv(W̄), Inf) < 1e-8
    λ = W * z
...
function jacobian(s, z; scaling = true)
    ...
    jac = [P A' D' zeros(nx,ns);
         ...
         zeros(ns,nx) zeros(ns,nb) mλ*W    mλ*inv(W)';
    ]
...
function correction(s, z, Δsa, Δza, σ, μ, res; scaling = true)
    jac = jacobian(s, z, scaling = scaling)
    n = length(s)
    e = cone_neutral(n)
    λ = nd_λ(s, z)
    W = nd_scaling(s, z)
    Δ = - jac \ [(1-σ)*res; cone_prod(λ, λ) + cone_prod(inv(W)'*Δsa, W*Δza) - σ*μ*e]
    uΔ = - jac \ [(1-σ)*res; cone_prod(s, z) + cone_prod(Δsa, Δza) - σ*μ*e]
    scaling && return Δ
    return uΔ
end
