...
@inline function input_jacobian_configuration_parent(joint::Translational{T}, statea::State, stateb::State) where T
    ...
    xb, qb = current_configuration(stateb)
    # d[Faw;2τaa]/d[xa,qa]
    ∇aa = impulse_transform_parent_jacobian_parent(joint, xa, qa, xb, qb, joint.input)
    FaXa = ∇aa[1:3,1:3]
    FaQa = ∇aa[1:3,4:6]
    τaXa = 0.5*∇aa[4:6,1:3]
    τaQa = 0.5*∇aa[4:6,4:6]
    # d[Fbw;2τbb]/d[xa,qa]
    ∇ba = impulse_transform_child_jacobian_parent(joint, xa, qa, xb, qb, joint.input)
    FbXa = ∇ba[1:3,1:3]
    FbQa = ∇ba[1:3,4:6]
    τbXa = 0.5*∇ba[4:6,1:3]
    τbQa = 0.5*∇ba[4:6,4:6]
...
@inline function input_jacobian_configuration_child(joint::Translational{T}, statea::State, stateb::State) where T
    ...
    xb, qb = current_configuration(stateb)
    # d[Faw;2τaa]/d[xb,qb]
    ∇ab = impulse_transform_parent_jacobian_child(joint, xa, qa, xb, qb, joint.input)
    FaXb = ∇ab[1:3,1:3]
    FaQb = ∇ab[1:3,4:6]
    τaXb = 0.5*∇ab[4:6,1:3]
    τaQb = 0.5*∇ab[4:6,4:6]
    # d[Fbw;2τbb]/d[xb,qb]
    ∇bb = impulse_transform_child_jacobian_child(joint, xa, qa, xb, qb, joint.input)
    FbXb = ∇bb[1:3,1:3]
    FbQb = ∇bb[1:3,4:6]
    τbXb = 0.5*∇bb[4:6,1:3]
    τbQb = 0.5*∇bb[4:6,4:6]
