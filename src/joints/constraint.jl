mutable struct JointConstraint{T,N,Nc,Cs} <: Constraint{T,N}
    ...
    function JointConstraint(data; name::Symbol=Symbol("joint_" * randstring(4)))
        ...
        return new{T,N,Nc,typeof(constraints)}(getGlobalID(), name, spring, damper, constraints, parent_id, child_ids, minimal_index, variables)
    end
end
...
function set_joint_position!(mechanism, joint::JointConstraint{T,N,Nc}, xθ) where {T,N,Nc}
    ...
    for i = 1:n
        body2 = get_body(mechanism, joint.child_ids[i])
...
function set_velocity!(mechanism, joint::JointConstraint{T,N,Nc}, vω) where {T,N,Nc}
    ...
    for i = 1:n
        body2 = get_body(mechanism, joint.child_ids[i])
        Δv = get_velocity_delta(joint.constraints[i], body1, body2, vω[SUnitRange(joint.minimal_index[i][1], joint.minimal_index[i][2])]) # projection in body1 frame
        Δω = get_velocity_delta(joint.constraints[i+1], body1, body2, vω[SUnitRange(joint.minimal_index[i+1][1], joint.minimal_index[i+1][2])]) # projection in body1 frame
        p1, p2 = joint.constraints[i].vertices
...
@generated function minimal_coordinates(mechanism, joint::JointConstraint{T,N,Nc}) where {T,N,Nc}
    vec = [:(minimal_coordinates(joint.constraints[$i], get_body(mechanism, joint.parent_id), get_body(mechanism, joint.child_ids[$i]))) for i = 1:Nc]
    return :(svcat($(vec...)))
end

@generated function minimal_velocities(mechanism, joint::JointConstraint{T,N,Nc}) where {T,N,Nc}
    vec = [:(minimal_velocities(joint.constraints[$i], get_body(mechanism, joint.parent_id), get_body(mechanism, joint.child_ids[$i]))) for i = 1:Nc]
    return :(svcat($(vec...)))
end
...
@inline function impulses_jacobian_velocity!(mechanism, body::Body, joint::JointConstraint{T,N,Nc}) where {T,N,Nc}
    ...
    elseif body.id ∈ joint.child_ids
...
@generated function constraint(mechanism, joint::JointConstraint{T,N,Nc,Cs}) where {T,N,Nc,Cs}
    vec = [:(constraint(joint.constraints[$i], get_body(mechanism, joint.parent_id), get_body(mechanism, joint.child_ids[$i]), joint.variables[2][λindex(joint,$i)], mechanism.timestep)) for i = 1:Nc]
    return :(svcat($(vec...)))
end
...
@generated function impulse_map_parent(mechanism, joint::JointConstraint{T,N,Nc}, body::Body) where {T,N,Nc}
    vec = [:(impulse_map_parent(joint.constraints[$i], body, get_body(mechanism, joint.child_ids[$i]), joint.child_ids[$i], joint.variables[2][λindex(joint,$i)], mechanism.timestep)) for i = 1:Nc]
    return :(hcat($(vec...)))
end

@generated function impulse_map_child(mechanism, joint::JointConstraint{T,N,Nc}, body::Body) where {T,N,Nc}
    vec = [:(impulse_map_child(joint.constraints[$i], get_body(mechanism, joint.parent_id), body, joint.child_ids[$i], joint.variables[2][λindex(joint,$i)], mechanism.timestep)) for i = 1:Nc]
    return :(hcat($(vec...)))
end
...
@generated function constraint_jacobian_parent(mechanism, joint::JointConstraint{T,N,Nc}, body::Body) where {T,N,Nc}
    vec = [:(constraint_jacobian_parent(joint.constraints[$i], body, get_body(mechanism, joint.child_ids[$i]), joint.child_ids[$i], joint.variables[2][λindex(joint,$i)], mechanism.timestep)) for i = 1:Nc]
    return :(vcat($(vec...)))
end

@generated function constraint_jacobian_child(mechanism, joint::JointConstraint{T,N,Nc,Cs}, body::Body) where {T,N,Nc,Cs}
    vec = [:(constraint_jacobian_child(joint.constraints[$i], get_body(mechanism, joint.parent_id), body, joint.child_ids[$i], joint.variables[2][λindex(joint,$i)], mechanism.timestep)) for i = 1:Nc]
    return :(vcat($(vec...)))
end
...
@inline function spring_parent(mechanism, joint::JointConstraint{T,N,Nc}, body::Body; unitary::Bool=false) where {T,N,Nc}
    vec = szeros(T,6)
    for i=1:Nc
        vec += spring_parent(joint.constraints[i], body, get_body(mechanism, joint.child_ids[i]), mechanism.timestep, joint.child_ids[i], unitary=unitary)
    end
    return vec
end
...
@inline function spring_child(mechanism, joint::JointConstraint{T,N,Nc}, body::Body; unitary::Bool=false) where {T,N,Nc}
    vec = szeros(T,6)
    for i=1:Nc
        vec += spring_child(joint.constraints[i], get_body(mechanism, joint.parent_id), body, mechanism.timestep, joint.child_ids[i], unitary=unitary)
    end
    return vec
end
...
@inline function damper_parent(mechanism, joint::JointConstraint{T,N,Nc}, body::Body; unitary::Bool=false) where {T,N,Nc}
    vec = szeros(T,6)
    for i=1:Nc
        vec += damper_parent(joint.constraints[i], body, get_body(mechanism, joint.child_ids[i]), mechanism.timestep, joint.child_ids[i], unitary=unitary)
    end
    return vec
end
...
@inline function damper_child(mechanism, joint::JointConstraint{T,N,Nc}, body::Body; unitary::Bool=false) where {T,N,Nc}
    vec = szeros(T,6)
    for i=1:Nc
        vec += damper_child(joint.constraints[i], get_body(mechanism, joint.parent_id), body, mechanism.timestep, joint.child_ids[i], unitary=unitary)
    end
    return vec
end
...
@generated function input_jacobian_control_parent(mechanism, joint::JointConstraint{T,N,Nc}, body::Body) where {T,N,Nc}
    vec = [:(input_jacobian_control_parent(joint.constraints[$i], body, get_body(mechanism, joint.child_ids[$i]), mechanism.timestep, joint.child_ids[$i])) for i = 1:Nc]
    return :(hcat($(vec...)))
end

@generated function input_jacobian_control_child(mechanism, joint::JointConstraint{T,N,Nc}, body::Body) where {T,N,Nc}
    vec = [:(input_jacobian_control_child(joint.constraints[$i], get_body(mechanism, joint.parent_id), body, mechanism.timestep, joint.child_ids[$i])) for i = 1:Nc]
    return :(hcat($(vec...)))
end

@inline function apply_input!(joint::JointConstraint{T,N,Nc}, mechanism, clear::Bool=true) where {T,N,Nc}
    for i=1:Nc
        apply_input!(joint.constraints[i], get_body(mechanism, joint.parent_id), get_body(mechanism, joint.child_ids[i]), mechanism.timestep, clear)
    end
    return
end
