...
p0 = rand(3)
impulse_transform(:parent, rot0, xa, qa, xb, qb)
impulse_transform(:child, rot0, xa, qa, xb, qb)

∇0 = impulse_transform_jacobian(:parent, :parent, rot0, xa, qa, xb, qb, p0)
∇1 = FiniteDiff.finite_difference_jacobian(
    xq -> impulse_transform(:parent, rot0, xq[1:3], UnitQuaternion(xq[4:7]..., false), xb, qb)*p0, [xa; vector(qa)]) *
    cat(I(3), LVᵀmat(qa), dims=(1,2))
@test norm(∇0 - ∇1, Inf) < 1e-7

∇0 = impulse_transform_jacobian(:parent, :child, rot0, xa, qa, xb, qb, p0)
∇1 = FiniteDiff.finite_difference_jacobian(
    xq -> impulse_transform(:parent, rot0, xa, qa, xq[1:3], UnitQuaternion(xq[4:7]..., false))*p0, [xb; vector(qb)]) *
    cat(I(3), LVᵀmat(qb), dims=(1,2))
@test norm(∇0 - ∇1, Inf) < 1e-7

∇0 = impulse_transform_jacobian(:child, :parent, rot0, xa, qa, xb, qb, p0)
∇1 = FiniteDiff.finite_difference_jacobian(
    xq -> impulse_transform(:child, rot0, xq[1:3], UnitQuaternion(xq[4:7]..., false), xb, qb)*p0, [xa; vector(qa)]) *
    cat(I(3), LVᵀmat(qa), dims=(1,2))
@test norm(∇0 - ∇1, Inf) < 1e-7

∇0 = impulse_transform_jacobian(:child, :child, rot0, xa, qa, xb, qb, p0)
∇1 = FiniteDiff.finite_difference_jacobian(
    xq -> impulse_transform(:child, rot0, xa, qa, xq[1:3], UnitQuaternion(xq[4:7]..., false))*p0, [xb; vector(qb)]) *
    cat(I(3), LVᵀmat(qb), dims=(1,2))
@test norm(∇0 - ∇1, Inf) < 1e-7
...
p0 = rand(3)
impulse_transform(:parent, tra0, xa, qa, xb, qb)

impulse_transform(:child, tra0, xa, qa, xb, qb)

∇0 = impulse_transform_jacobian(:parent, :parent, tra0, xa, qa, xb, qb, p0)
∇1 = FiniteDiff.finite_difference_jacobian(
    xq -> impulse_transform(:parent, tra0, xq[1:3], UnitQuaternion(xq[4:7]..., false), xb, qb)*p0, [xa; vector(qa)]) *
    cat(I(3), LVᵀmat(qa), dims=(1,2))
@test norm(∇0 - ∇1, Inf) < 1e-7

∇0 = impulse_transform_jacobian(:parent, :child, tra0, xa, qa, xb, qb, p0)
∇1 = FiniteDiff.finite_difference_jacobian(
    xq -> impulse_transform(:parent, tra0, xa, qa, xq[1:3], UnitQuaternion(xq[4:7]..., false))*p0, [xb; vector(qb)]) *
    cat(I(3), LVᵀmat(qb), dims=(1,2))
@test norm(∇0 - ∇1, Inf) < 1e-7

∇0 = impulse_transform_jacobian(:child, :parent, tra0, xa, qa, xb, qb, p0)
∇1 = FiniteDiff.finite_difference_jacobian(
    xq -> impulse_transform(:child, tra0, xq[1:3], UnitQuaternion(xq[4:7]..., false), xb, qb)*p0, [xa; vector(qa)]) *
    cat(I(3), LVᵀmat(qa), dims=(1,2))
@test norm(∇0 - ∇1, Inf) < 1e-7

∇0 = impulse_transform_jacobian(:child, :child, tra0, xa, qa, xb, qb, p0)
∇1 = FiniteDiff.finite_difference_jacobian(
    xq -> impulse_transform(:child, tra0, xa, qa, xq[1:3], UnitQuaternion(xq[4:7]..., false))*p0, [xb; vector(qb)]) *
    cat(I(3), LVᵀmat(qb), dims=(1,2))
@test norm(∇0 - ∇1, Inf) < 1e-7
...
transpose(impulse_transform(:parent, tra0, xa, qa, xb, qb))
...
transpose(impulse_transform(:child, tra0, xa, qa, xb, qb))
