...
"""
    ...
    opts: SolverOptions
"""
function get_body_contact_gradients!(mechanism::Mechanism{T,Nn,Ne,Nb,Ni},
		z::AbstractVector{T}, u::AbstractVector{T},
		θ::AbstractVector{T}; opts=SolverOptions()) where {T,Nn,Ne,Nb,Ni}
	z_next = body_contact_step!(mechanism, z, u, θ)
...
function body_contact_step!(mechanism::Mechanism{T,Nn,Ne,Nb,Ni},
		z::AbstractVector{T}, u::AbstractVector{T},
		θ::AbstractVector{T}; opts=SolverOptions()) where {T,Nn,Ne,Nb,Ni}
...
function loss(mechanism::Mechanism, θ::AbstractVector{T}, traj::Storage{T,N},
	...
	for i in indices
		...
		u_true = get_input(traj, i)
		Q = Diagonal(vcat(fill([ones(3); 1e-1ones(3); ones(4); 1e-1ones(3)], Nb)...))

		if derivatives
			z_pred, ∂_state, ∂_body, ∂_contact = get_body_contact_gradients!(mechanism, z, u_true, θ, opts=opts)
