...
function density_hessian(nerf::NerfObject{T}, p::AbstractVector{T}) where T
    # p and ∇ in Nerf frame
    ∇ = FiniteDiff.finite_difference_hessian(p -> density(nerf, p), p)
    return ∇
end
...
function constraint(mechanism, contact::ContactConstraint{T,N,Nc,Cs}) where {T,N,Nc,Cs<:Tuple{NerfContact{T,N}}}
    ...
    # cp = zeros(3)
    # for k = 1:10
    #     cp[1:2] += -0.2 * density_hessian(nerf, cp)[1:2,1:2] \ density_gradient(nerf, cp)[1:2]
    # end
    # r = - bound.p + inv(q3) * (cp - x3) # position of the origin point in the frame of centered on P expressed in the body's frame
    # c = bound.nerf.threshold - density(bound.nerf, r) # > 0
    c = bound.nerf.threshold + nerf_constraint(bound.nerf, bound.p, q3, x3)
    SVector{1,T}(c - contact.primal[2][1])
end

function nerf_constraint(nerf, p, q3, x3)
    cp = [x3[1:2]+0.05*rand(2); 0]
    for k = 1:100
        g = FiniteDiff.finite_difference_gradient(cp -> density(nerf, Vector(- p + inv(q3) * (cp - x3))), cp)
        H = FiniteDiff.finite_difference_hessian(cp -> density(nerf, Vector(- p + inv(q3) * (cp - x3))), cp)
        Δ = (H+0.05I) \ g
        cp += 0.95*[Δ[1:2]; 0.0]
        norm(Δ[1:2], Inf) < 1e-5
        # @show cp
...
@inline function constraint_jacobian_velocity(bound::NerfContact, x3::AbstractVector, q3::UnitQuaternion,
    ...
    FiniteDiff.finite_difference_jacobian(
        vϕ -> nerf_constraint(bound.nerf, bound.p,
            next_orientation(q2, vϕ[SUnitRange(4,6)], timestep),
            next_position(x2, vϕ[SUnitRange(1,3)], timestep)),
        [v25; ϕ25])
end
...
@inline function constraint_jacobian_configuration(bound::NerfContact, x3::AbstractVector, q3::UnitQuaternion,
    ...
    # return [X Q]
    FiniteDiff.finite_difference_jacobian(
        xq -> nerf_constraint(bound.nerf, bound.p, UnitQuaternion(xq[4:7]..., false), xq[1:3]),
        [x3; vector(q3)])
end
...
@inline function impulse_map(bound::NerfContact, x::AbstractVector, q::UnitQuaternion, λ)
    ...
    X = FiniteDiff.finite_difference_jacobian(
        x -> nerf_constraint(bound.nerf, bound.p, q, x),
        x)
    Q = FiniteDiff.finite_difference_jacobian(
        q -> nerf_constraint(bound.nerf, bound.p, UnitQuaternion(q..., false), x),
        vector(q)) * LVᵀmat(q)
...
initialize!(mech, :nerf, x=[0.00,0.01,0.35], v=[0.00, 0.12, 0.1])
...
# open(vis)
mech = get_box()
initialize!(mech, :box2d)
sotrage = simulate!(mech, 2.0, record=true, opts=SolverOptions(btol=1e-5))
visualize(mech, sotrage, vis=vis)
set_camera!(vis, cam_pos=[0,5,0], zoom=10)
a = 0
a = 0
a = 0
a = 0
