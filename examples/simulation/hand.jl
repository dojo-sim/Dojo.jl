...
palm_body = Capsule(palm_radius, palm_length, palm_mass,
...
links = [palm_body, thumb..., index..., middle..., ring..., pinky...]

# Joints
origin_palm_joint = JointConstraint(Floating(origin, palm_body))

# thumb
palm_thumb_a_joint = JointConstraint(Revolute(palm_body, thumb_a, [0.0; 1.0; 0.0]; 
    parent_vertex=[0.0; 0.75 * palm_length; -palm_radius], 
    child_vertex=[0.0; 0.0; segment_length],
    spring=spring, 
    damper=damper
    )
)
...
thumb_a_thumb_b_joint = JointConstraint(Revolute(thumb_a, thumb_b, [0.0; 1.0; 0.0]; 
    parent_vertex=[0.0; 0.0; -1.0 * segment_length], 
    child_vertex=[0.0; 0.0; 1.0 * segment_length],
    spring=spring, 
    damper=damper
    )
)
...
# index
palm_index_a_joint = JointConstraint(Revolute(palm_body, index_a, [0.0; 1.0; 0.0]; 
    parent_vertex=[0.0; 0.75 * palm_length; palm_radius], 
    child_vertex=[0.0; 0.0; -segment_length],
    spring=spring, 
    damper=damper
...
# middle
palm_middle_a_joint = JointConstraint(Revolute(palm_body, middle_a, [0.0; 1.0; 0.0]; 
...
# ring
palm_ring_a_joint = JointConstraint(Revolute(palm_body, ring_a, [0.0; 1.0; 0.0]; 
    parent_vertex=[0.0; -0.25 * palm_length; palm_radius], 
    child_vertex=[0.0; 0.0; -segment_length],
    spring=spring, 
    damper=damper
...
# pinky
palm_pinky_a_joint = JointConstraint(Revolute(palm_body, pinky_a, [0.0; 1.0; 0.0]; 
    parent_vertex=[0.0; -0.75 * palm_length; palm_radius], 
    child_vertex=[0.0; 0.0; -segment_length],
    spring=spring, 
    damper=damper
...
joints = [origin_palm_joint, thumb_joints..., index_joints..., middle_joints..., ring_joints..., pinky_joints...] 
...
hand_mechanism.bodies[3].state.x2 = [0.0; 0.5 * palm_length; -1.0 * (0.5 * segment_length + palm_radius + 1.0 * segment_length)]
hand_mechanism.bodies[4].state.x2 = [0.0; 0.5 * palm_length; -1.0 * (0.5 * segment_length + palm_radius + 2.0 * segment_length)]

# index
hand_mechanism.bodies[5].state.x2 = [0.0; 0.75 * palm_length; 1.0 * (0.5 * segment_length + palm_radius)]
hand_mechanism.bodies[6].state.x2 = [0.0; 0.75 * palm_length; 1.0 * (0.5 * segment_length + palm_radius + 1.0 * segment_length)]
hand_mechanism.bodies[7].state.x2 = [0.0; 0.75 * palm_length; 1.0 * (0.5 * segment_length + palm_radius + 2.0 * segment_length)]

# middle
hand_mechanism.bodies[8].state.x2 = [0.0; 0.25 * palm_length; 1.0 * (0.5 * segment_length + palm_radius)]
hand_mechanism.bodies[9].state.x2 = [0.0; 0.25 * palm_length; 1.0 * (0.5 * segment_length + palm_radius + 1.0 * segment_length)]
hand_mechanism.bodies[10].state.x2 = [0.0; 0.25 * palm_length; 1.0 * (0.5 * segment_length + palm_radius + 2.0 * segment_length)]

# middle
hand_mechanism.bodies[11].state.x2 = [0.0; -0.25 * palm_length; 1.0 * (0.5 * segment_length + palm_radius)]
hand_mechanism.bodies[12].state.x2 = [0.0; -0.25 * palm_length; 1.0 * (0.5 * segment_length + palm_radius + 1.0 * segment_length)]
hand_mechanism.bodies[13].state.x2 = [0.0; -0.25 * palm_length; 1.0 * (0.5 * segment_length + palm_radius + 2.0 * segment_length)]

# pinky 
hand_mechanism.bodies[14].state.x2 = [0.0; -0.75 * palm_length; 1.0 * (0.5 * segment_length + palm_radius)]
hand_mechanism.bodies[15].state.x2 = [0.0; -0.75 * palm_length; 1.0 * (0.5 * segment_length + palm_radius + 1.0 * segment_length)]
hand_mechanism.bodies[16].state.x2 = [0.0; -0.75 * palm_length; 1.0 * (0.5 * segment_length + palm_radius + 2.0 * segment_length)]
