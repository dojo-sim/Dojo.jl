...
env = quadruped(
    ...
    spring=spring,
	infeasible_control=true,
	opts_step=SolverOptions(rtol=1.0e-4, btol=1.0e-4, undercut=1.5),
    opts_grad=SolverOptions(rtol=1.0e-4, btol=1.0e-4, undercut=1.5)
	)

# ## rollout
x1 = xref[1]
ū2 = u_sol
w2 = [zeros(d) for t = 1:T-1]
x̄2 = IterativeLQR.rollout(model, x1, ū2, w2)
visualize(env, x̄2)
...
# ## problem
prob = IterativeLQR.problem_data(model, obj, cons)
IterativeLQR.initialize_controls!(prob, ū2)
IterativeLQR.initialize_states!(prob, x̄2)
