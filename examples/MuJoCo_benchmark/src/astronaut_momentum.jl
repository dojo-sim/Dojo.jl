...
function mj_astronaut_simulation(; timestep=0.01, tsim=1.0, tctrl=1.0, seed::Int=0, ϵ=1e-14,
	...
	traj = zeros(83,0)
	for i = 1:N
		jd.ctrl .= (i*timestep < tctrl) * control_amplitude * rand(21)
		tcompute += @elapsed mj_step(jm, jd)
		record_traj && (traj = hcat(traj, deepcopy(getstate(mjsim))))
		if !(i*timestep < tctrl) && ((i-1)*timestep < tctrl)
			# first step after the end of control inputs
			ener0 = energy(jm, jd)
		end
		if record_ener && !(i*timestep < tctrl) && (i % Int(floor(1/timestep)) == 0)
...
function mj_batch_astronaut_simulation(; Nsim=1, timestep=0.01, tsim=1.0, tctrl=1.0, seed::Int=0, ϵ=1e-14,
	...
	fail = 0.0
	for i = 1:Nsim
		tcompute, pl, pa, en, fa, _ = mj_astronaut_simulation(; timestep=timestep, tsim=tsim, tctrl=tctrl, seed=seed, ϵ=ϵ,
...
function mj_benchmark_momentum(timestep; Nsim=1, tsim=1.0, tctrl=1.0, seed::Int=0, ϵ=1e-14,
		control_amplitude=0.05, record_traj::Bool=false)
	N = length(timestep)
	speed = zeros(N)
	plin = zeros(N)
	...
	for i = 1:N
		@show timestep[i]
		speed[i], plin[i], pang[i], ener[i], fail[i] = mj_batch_astronaut_simulation(; Nsim=Nsim, timestep=timestep[i],
...
speed, plin, pang, Δener, fail, traj, ener_traj = mj_astronaut_simulation(tsim=1, tctrl=1.0,
	timestep=0.1, control_amplitude=0.02)
...
for i = 1:3
	speed, plin, pang, Δener, fail, traj, ener_t = mj_astronaut_simulation(tsim=100, tctrl=1.0,
		timestep=timestep[i], control_amplitude=0.02, record_traj=false, record_ener=true)
