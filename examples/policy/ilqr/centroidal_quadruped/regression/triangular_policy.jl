...
∂policy∂xθ0 = FiniteDiff.finite_difference_jacobian(θ -> reshape(linear_policy_jacobian_state(θ, 2), nu*2nx), rand(nu*2nx))
∂policy∂xθ = I(nu*2nx)
function linear_policy_jacobian_state_parameters()
    return sparse(I(nu*2nx))
end
...
function linear_policy_jacobian_last_state_parameters(i)
    return [spzeros(nu*nx, nu*(i-1)*nx) I(nu*1nx)]
end

function linear_policy_jacobian_parameters(x, x_ref, i)
    Δx = x - x_ref
    ∂policy∂θ = hcat([Δx[i] * I(nu) for i=1:i*nx]...)
    return ∂policy∂θ
end
...
function evaluation(θ, i, x_ref, u_ref, K_ref, x_sols, u_sols, K_sols;
        Qu=1e-0*I(nu)/nu, Qθ=1e-10*I(nu*i*nx)/nu/nx/i, QK=QK0*I(nu*nx)/nu/nx)

    sample_number = length(x_sols)
    c = 0.0

    for j = 1:sample_number
        x_sol = vcat(x_sols[j][1:i]...)
        u_sol = u_sols[j][i]
        K_sol = K_sols[j][i]
        # x_sol = x_ref[i]
        # u_sol = u_ref[i]
...
QK0 = 1e-4
K_fit = [zeros(nu,i*nx) for i=1:T-1]
for i = 1:T-1
    θ = zeros(nu*i*nx)
...
for i = 1:M
    for j = 1:T-1
        t += 1
        t_loop = (t-1)%(T-1)+1
        x_offset = vcat(deepcopy(x_ref[1:t_loop])...)
        ...
        push!(u_hist, linear_policy(vcat(x_hist[end-j+1:end]...), K_fit[j], x_offset, u_ref[t_loop], j))
