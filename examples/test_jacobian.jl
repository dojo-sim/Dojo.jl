...
dBqω_sym = Symbolics.jacobian(Bqω_sym, q) * G(q)
dBqω_sym = simplify.(dBqω_sym)

println(dBqω_sym[1, 1])
println(dBqω_sym[2, 1])

println(dBqω_sym[1, 2])
println(dBqω_sym[2, 2])

println(dBqω_sym[1, 3])
println(dBqω_sym[2, 3])
...
@test norm(dBqω(q0, ω0, p0) - ForwardDiff.jacobian(a -> Bqω(a, ω0, p0), q0) * G(q0)) < 1.0e-8
