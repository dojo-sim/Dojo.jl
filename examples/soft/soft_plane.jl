...
function constraint(mechanism, contact::ContactConstraint{T,N,Nc,Cs}) where {T,N,Nc,Cs<:Tuple{NerfContact{T,N}}}
    ...
    # end
    # r = - model.contact_point + inv(q3) * (cp - x3) # position of the origin point in the frame of centered on P expressed in the body's frame
    # c = model.nerf.threshold - density(model.nerf, r) # > 0
    c = model.nerf.threshold + nerf_constraint(model.nerf, model.contact_point, q3, x3)
...
@inline function impulse_map(model::NerfContact, x::AbstractVector, q::UnitQuaternion, λ)
    ...
    Q = FiniteDiff.finite_difference_jacobian(
        q -> nerf_constraint(model.nerf, model.contact_point, UnitQuaternion(q..., false), x),
        vector(q)) * LVᵀmat(q)
...
@inline function set_matrix_vector_entries!(mechanism::Mechanism, matrix_entry::Entry, vector_entry::Entry,
    contact::RigidContactConstraint{T,N,Nc,Cs,N½}) where {T,N,Nc,Cs<:Tuple{NerfContact{T,N}},N½}
