...
function setdata!(mechanism::Mechanism, data::AbstractVector)
    ...
    for eqc in mechanism.eqconstraints
        ...
        if dim > 0
            ...
            @show u
            setForce!(mechanism, eqc, SVector{dim}(u))
        end
    end
    foreach(applyFτ!, mechanism.eqconstraints, mechanism)
    return nothing
end

function getnextstate(mechanism::Mechanism{T}) where T
    data = Vector{T}()
    for body in mechanism.bodies
        x2 = body.state.xsol[2]
        v1 = body.state.vsol[2]
        qc = body.state.qsol[2]
