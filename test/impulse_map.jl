...
@testset "Displacement Jacobian" begin
    @testset "Rotational" begin
        ...
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.displacement(rot0, z[1:3], Quaternion(z[4:7]...,true), xb, qb),
            [xa; Dojo.vector(qa)]
            ) * attjac
        ...
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.displacement(rot0, xa, qa, z[1:3], Quaternion(z[4:7]...,true)),
            [xb; Dojo.vector(qb)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
    end
    ...
    @testset "Translational" begin
        ...
        attjac = cat(I(3),Dojo.LVᵀmat(qa), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.displacement(tra0, z[1:3], Quaternion(z[4:7]...,true), xb, qb),
            [xa; Dojo.vector(qa)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3),Dojo.LVᵀmat(qb), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.displacement(tra0, xa, qa, z[1:3], Quaternion(z[4:7]...,true)),
            [xb; Dojo.vector(qb)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
    end
...
@testset "Impulse transform Jacobian: rotational" begin
    ...
    @testset "Translational" begin
        ...
        attjac = cat(I(3),Dojo.LVᵀmat(qa), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_transform(:parent, tra0, z[1:3], Quaternion(z[4:7]...,true), xb, qb) * p0,
            [xa; Dojo.vector(qa)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3), Dojo.LVᵀmat(qb), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_transform(:parent, tra0, xa, qa, z[1:3], Quaternion(z[4:7]...,true)) * p0,
            [xb; Dojo.vector(qb)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3), Dojo.LVᵀmat(qa), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_transform(:child, tra0, z[1:3], Quaternion(z[4:7]...,true), xb, qb) * p0,
            [xa; Dojo.vector(qa)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3),Dojo.LVᵀmat(qb), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_transform(:child, tra0, xa, qa, z[1:3], Quaternion(z[4:7]...,true)) * p0,
            [xb; Dojo.vector(qb)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
    end
...
@testset "Impulse map" begin
    @testset "impulse map: rotational" begin
        ...
        attjac = cat(I(3),Dojo.LVᵀmat(qa), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_map(:parent, rot0, z[1:3], Quaternion(z[4:7]...,true), xb, qb, 0) * λ,
            [xa; Dojo.vector(qa)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3), Dojo.LVᵀmat(qb), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_map(:parent, rot0, xa, qa, z[1:3], Quaternion(z[4:7]...,true), 0) * λ,
            [xb; Dojo.vector(qb)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3), Dojo.LVᵀmat(qa), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_map(:child, rot0, z[1:3], Quaternion(z[4:7]...,true), xb, qb, 0) * λ,
            [xa; Dojo.vector(qa)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3), Dojo.LVᵀmat(qb), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_map(:child, rot0, xa, qa, z[1:3], Quaternion(z[4:7]...,true), 0) * λ,
            [xb; Dojo.vector(qb)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
    end
    ...
    @testset "Translational" begin
        ...
        attjac = cat(I(3),Dojo.LVᵀmat(qa), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_map(:parent, tra0, z[1:3], Quaternion(z[4:7]...,true), xb, qb, 0) * λ,
            [xa; Dojo.vector(qa)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3), Dojo.LVᵀmat(qb), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_map(:parent, tra0, xa, qa, z[1:3], Quaternion(z[4:7]...,true), 0) * λ,
            [xb; Dojo.vector(qb)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3), Dojo.LVᵀmat(qa), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_map(:child, tra0, z[1:3], Quaternion(z[4:7]...,true), xb, qb, 0) * λ,
            [xa; Dojo.vector(qa)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
        ...
        attjac = cat(I(3), Dojo.LVᵀmat(qb), dims=(1,2))
        J1 = FiniteDiff.finite_difference_jacobian(
            z -> Dojo.impulse_map(:child, tra0, xa, qa, z[1:3], Quaternion(z[4:7]...,true), 0) * λ,
            [xb; Dojo.vector(qb)]
            ) * attjac
        @test norm(J0 - J1, Inf) < 1.0e-7
    end
...
# joint0 = mech.joints[1]
# rot0 = joint0.rotational
# rot0.axis_offset = Quaternion(1,0,0,0.0)#rand(QuatRotation).q
#
#
# # xa = rand(3)
# xa = szeros(3)
# qa = rand(QuatRotation).q
# # qa = Quaternion(1,0,0,0.0,true)
# # xb = rand(3)
# xb = szeros(3)
# # qb = rand(QuatRotation).q
# qb = Quaternion(1,0,0,0.0,true)
# # λ = rand(2)
# λ = sones(2)
# η = rand(0)
# Dojo.impulse_transform(:parent, rot0, xa, qa, xb, qb)
# Dojo.impulse_transform(:child, rot0, xa, qa, xb, qb)
# Dojo.impulse_projector(rot0)
# Dojo.impulse_map(:parent, rot0, xa, qa, xb, qb, η)
# Dojo.impulse_map(:child, rot0, xa, qa, xb, qb, η)
#
# m = 1.0
