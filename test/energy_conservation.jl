...
m0 = momentum(mech, storage)[start0:end]
...
ke1 = kineticEnergy(mech, storage)
pe1 = potentialEnergy(mech, storage)
me1 = mechanicalEnergy(mech, storage)
plot([(i-1)*Δt0 for i in 1:length(m0)], hcat(mlin0...)')
plot([(i-1)*Δt0 for i in 1:length(m0)], hcat(mang0...)')
plot([(i-1)*Δt0 for i in 1:length(m0)], ke0 .- ke0[start0])
plot([(i-1)*Δt0 for i in 1:length(m0)], pe0 .- pe0[start0])
plot([(i-1)*Δt0 for i in 1:length(m0)], me0 .- me0[start0])
plot([(i-1)*Δt0 for i in 1:length(ke1)], ke1)
plot([(i-1)*Δt0 for i in 1:length(pe1)], pe1)
plot([(i-1)*Δt0 for i in 1:length(me1)], me1)
@test all(norm.(mlin0, Inf) .< 1e-11)
@test all(norm.(mang0, Inf) .< 1e-11)
@test norm(ke0 .- ke0[1], Inf) < 1e-11
@test norm(pe0 .- pe0[1], Inf) < 1e-11
@test norm(me0 .- me0[1], Inf) < 1e-11
...
pe0 = potentialEnergy(mech, storage)[start0:end]
me0 = mechanicalEnergy(mech, storage)[start0:end]
ke1 = kineticEnergy(mech, storage)[1:end]
pe1 = potentialEnergy(mech, storage)[1:end]
me1 = mechanicalEnergy(mech, storage)[1:end]
plot([(i-1)*Δt0 for i in 1:length(m0)], hcat(mlin0...)')
plot([(i-1)*Δt0 for i in 1:length(m0)], hcat(mang0...)')
plot([(i-1)*Δt0 for i in 1:length(m0)], ke0 .- ke0[start0])
plot([(i-1)*Δt0 for i in 1:length(m0)], pe0 .- pe0[start0])
...
ωl = minimum([ωl[1] for ωl in storage.ωl[1]])
vl = minimum([vl[2] for vl in storage.vl[1]])

pe2 = body1.m * -g0 * -0.5
ke2 = 0.5 * body1.m * vl^2
ke2 = 0.5 * body1.J[1,1] * ωl^2
ke2 = 0.5 * body1.J[1,1] * ωl^2 + 0.5 * body1.m * vl^2

plot([vl[3] for vl in storage.vl[1]])
plot!([vl[2] for vl in storage.vl[1]])
...
storage = simulate!(mech, 0.712,  nocontrol!, record = true, solver = :mehrotra!, verbose = false, ϵ = ϵ0)
visualize(mech, storage, vis = vis)
sqrt(-g0)
...
plot([(i-1)*Δt0 for i in 1:length(ke1)], ke1)
plot([(i-1)*Δt0 for i in 1:length(pe1)], pe1)
scatter([(i-1)*Δt0 for i in 1:length(me1)], me1, legend = false)
@test all(norm.(mlin0, Inf) .< 1e-11)
@test all(norm.(mang0, Inf) .< 1e-11)
@test norm(ke0 .- ke0[1], Inf) < 1e-11
@test norm(pe0 .- pe0[1], Inf) < 1e-11
...
body1 = collect(mech.bodies)[1]
vl2 = minimum([vl[3] for vl in storage.vl[1]])
x2 = minimum([x[3] for x in storage.x[1]])
x3 = 0.00
vl3 = 0.00

pe2 = body1.m * -g0 * x2 + 0.5 * spring0 * (x2/2)^2
ke2 = 0.5 * body1.m * (vl2-0.0000712)^2
me2 = pe2 + ke2

pe3 = body1.m * -g0 * x3 + 0.5 * spring0 * x3^2
ke3 = 0.5 * body1.m * vl3^2
me3 = pe3 + ke3

plot([vl[3] for vl in storage.vl[1]] - [v[3] for v in storage.v[1]])
plot!([v[3] for v in storage.v[1]])
plot!([vl[2] for vl in storage.vl[1]])
...
mang0 = [Vector(m-m0[1])[4:6] for m in m0]
ke0 = kineticEnergy(mech, storage)[start0:end]
pe0 = kineticEnergy(mech, storage)[start0:end]
plot([(i-1)*Δt0 for i in 1:length(m0)], hcat(mlin0...)')
plot([(i-1)*Δt0 for i in 1:length(m0)], hcat(mang0...)')
plot([(i-1)*Δt0 for i in 1:length(m0)], ke0 .- ke0[start0])
@test all(norm.(mlin0, Inf) .< 1e-11)
