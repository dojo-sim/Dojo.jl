...
function getenergy(model::Symbol, t::T, Δt::T, g::T, ϵ::T, controller!::Any; mech_kwargs::Dict = Dict(), init_kwargs::Dict = Dict(), complete = false) where T
    ...
    me = DifferentiableContact.mechanicalEnergy(mechanism)
    pe = DifferentiableContact.potentialEnergy(mechanism)
    ke = DifferentiableContact.kineticEnergy(mechanism)
    m = DifferentiableContact.momentum(mechanism)
    complete && (return [me; pe; ke; m])
    return me
end
...
# ms = ms .- ms[1]
plot(tss, ms, label = "energy", title = "mechanical energy")
@testset "Humanoid" begin @test all(norm.(ms, Inf) .< 1e-11) end
...
tss = 0:0.5:5
ms = getenergy.(:atlas, tss, Δt0, g0, ϵ0, controller!;
    mech_kwargs = Dict(:contact => false, :spring => spring0, :damper => damper0))
# ms = ms .- ms[1]
plot(tss, ms, label = "energy", title = "mechanical energy")
@testset "Atlas" begin @test all(norm.(ms, Inf) .< 1e-11) end
...
function controller_quadruped!(mechanism, k; U = 0.1, Δt = 0.01)
    for (i,joint) in enumerate(mechanism.eqconstraints)
        nu = getcontroldim(joint)
        u = (nu <= 5 && k ∈ (1:100)) * U * Δt * sones(nu)
        setForce!(mechanism, joint, u)
...
tss = 0:0.5:5
ms = getenergy.(:quadruped, tss, Δt0, g0, ϵ0, controller_quadruped!;
    mech_kwargs = Dict(:contact => false, :spring => spring0, :damper => damper0))
# ms = ms .- ms[1]
plot(tss, ms, label = "energy", title = "mechanical energy")
@testset "Quadruped" begin @test all(norm.(ms, Inf) .< 1e-11) end
...
@testset "Snake" begin
    for jointtype in jointtypes
        ms = getenergy.(:snake, ts, Δt0, g0, ϵ0;
