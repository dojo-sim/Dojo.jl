...
def extract_density(xyz, kwargs):
    ...
    model = kwargs['object_params'][0]['network_fine'] # even better possibly 
...
def density_query(render_kwargs_test, np_xyz):
    with torch.no_grad():
        xyz = torch.from_numpy(np_xyz)
        density = extract_density(xyz=xyz, kwargs=render_kwargs_test)
    return density.cpu().numpy()

def density_gradient_query(render_kwargs_test, np_xyz):
    xyz = torch.from_numpy(np_xyz)
    xyz.requires_grad_()
    density = extract_density(xyz=xyz, kwargs=render_kwargs_test)
    density.sum().backward()
    return xyz.grad.cpu().numpy()
