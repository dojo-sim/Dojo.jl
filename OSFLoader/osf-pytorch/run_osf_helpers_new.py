...
def run_single_object(
    ...
    Returns:
        ...
        ret0: [Dict]. Results from the coarse model.
            ...
            If retraw is True, we also provide:
                ...
                pts
    """
    # Extract ray origin, direction, near, far, normalized direction, and img_id.
    rays_o, rays_d = ray_batch[:, 0:3], ray_batch[:, 3:6]  # [R, 3] each
    near, far = ray_batch[:, 6][:, None], ray_batch[:, 7][:, None]
    rays_d_norm = ray_batch[:, 8:11]
    rays_i = ray_batch[:, -1:]
    ...
    # Evaluate model at each point.
    raw1 = network_query_fn(pts, viewdirs, lightdirs, network_fn)  # [R, S, 4]
    rgb1, alpha1 = normalize_raw(raw1, z_vals, rays_d, raw_noise_std, scaled_sigmoid)

    raw2 = network_query_fn(pts, viewdirs, viewdirs, network_fn)
    rgb2, alpha2 = normalize_raw(raw2, z_vals, rays_d, raw_noise_std, scaled_sigmoid)

    # Calculate new ray_batch for the second light source.
    t = (-rays_o[:, -1] / rays_d_norm[:, -1])[:, None]
    rays_o_ = rays_o + t*rays_d_norm
    rays_d_ = metadata[rays_i.long()].squeeze() - rays_o_
    near_ = torch.zeros_like(near)
    far_ = torch.full(far.shape, 4.0)
    rays_d_norm_ = rays_d_ / torch.linalg.norm(rays_d_, dim=1)[:, None]
    rays_i_ = rays_i
    ray_batch_ = torch.cat((rays_o_, rays_d_, near_, far_, rays_d_norm_, rays_i_), dim=1)
    second_ret0, second_ret = run_second_light_source(
        ray_batch=ray_batch_,
        metadata=metadata,
        network_fn=network_fn,
        network_query_fn=network_query_fn,
        use_viewdirs=use_viewdirs,
        use_lightdirs=use_lightdirs,
        ...
        verbose=verbose,
        kwargs=kwargs)
    second_light_rgb_map = second_ret0['rgb']
    rgb2 = rgb2 * second_light_rgb_map * rays_d_norm_[:, -1][:, None, None] * torch.tensor(floor_color, dtype=torch.float32, requires_grad=True)

    # Calculate the rgb_map of the second light source.
    ...
    if N_importance > 0:
        ...
        # Sample fine points along each ray.
        weights = compute_weights(alpha=alpha1[..., 0])  # [R, S]
        z_vals, z_samples, pts = default_ray_sampling_fine(
            ray_batch, z_vals, weights, perturb, N_importance)
        viewdirs, lightdirs = get_dirs(
                ray_batch, pts, metadata, use_viewdirs, use_lightdirs, lightdirs_method)
        ...
        # Make predictions with network_fine.
        run_fn = network_fn if network_fine is None else network_fine
        raw1 = network_query_fn(pts, viewdirs, lightdirs, run_fn)
        rgb1, alpha1 = normalize_raw(raw1, z_vals, rays_d, raw_noise_std, scaled_sigmoid)
        ...
        second_ret0, second_ret = run_second_light_source(
            ...
            verbose=verbose,
            kwargs=kwargs)
        second_light_rgb_map = second_ret['rgb']
        rgb2 = rgb2 * second_light_rgb_map * rays_d_norm_[:, -1][:, None, None] * torch.tensor(floor_color, dtype=torch.float32, requires_grad=True)
