...
def run_network(inputs, viewdirs, lightdirs, fn, embed_fn, embedviews_fn, embedlights_fn, netchunk=1024*64):
    ...
    print("lightdirs", lightdirs)
    if viewdirs is not None:
        embedded_dirs = prepare_and_embed(embedviews_fn, viewdirs)
        embedded = torch.cat((embedded, embedded_dirs), dim=-1)
    if lightdirs is not None and embedlights_fn is not None:
...
def extract_density(xyz, kwargs):
    ...
    # model = kwargs['object_params'][0]['network_fine']
    model = kwargs['object_params'][0]['network_fn'] + kwargs['object_params'][0]['network_fine'] # even better possibly 
...
def demo():
    ...
    args = parser.parse_args(args=['--config', '/home/simon/.julia/dev/Dojo.jl/OSFLoader/osf-pytorch/configs/osf/bluesoap/bluesoap.txt'])
    ...
    with torch.no_grad():
        ...
        # xyz[:, 1] = 0.613254
        # xyz[:, 2] = 0.613254
        # -2.047862 0.613254 0.613254
        # xyz[:, 0] = -2.535888
        # xyz[:, 1] = 0.640379
        # xyz[:, 2] = 4.391862
        # -2.535888 0.640379 4.391862
        # xyz[:, 0] = -2.055238
        # xyz[:, 1] = 1.555518
        # xyz[:, 2] = 3.483968
        # -2.055238 1.555518 3.483968
        xyz[:, 0] = -0.0207
        xyz[:, 1] = 2.5801
        xyz[:, 2] = 4.9728
